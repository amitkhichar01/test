...
function buildSearchList(resid, skey, list) {
    ...
    if (list.length == 0) {
        document.getElementById(outputbox).innerHTML =
            `<div class="search_noresults"><p>No results for '` + skey + `'</p><p>We couldn't find anything matching your search. Try again with a different term.</p></div>`;
        return false;
    }
...
$(document).ready(function () {
    // == .custmonDropDwon == //
    let custmonDropDwon = document.querySelectorAll(".custmonDropDwon");
    custmonDropDwon.addEventListener("click", (e) => {
        ...
        let customDrp = e.target.nextElementSibling.classList.contains(".customDrp");
        if (customDrp.length) {
            customDrp.classList.add("active");
        } else {
            e.target.children(".customDrp").classList.add("active");
        }
    ...
    // filterToggelBtn
    //error on this line $(".filterToggel").slideToggle("active");

    document.querySelectorAll(".filterToggelBtn").addEventListener("click", () => {
        document.querySelectorAll(".filterToggel").classList.toggle("active");
    });
    ...
    // ==  .hideCustomDrp == //
    document.querySelectorAll(".hideCustomDrp").addEventListener("click", () => {
    ...
    document.addEventListener(
        ...
        function (e) {
            ...
            if (target.hasAttribute("data-toggle") && target.getAttribute("data-toggle") == "customPopup") {
                if (target.hasAttribute("data-target")) {
                    ...
                    if (m_ID == "LoginBox") {
                        ...
                        signupError.style.innerHTML = " ";

                        // $("#signup_error").css("display", "none");
                        // $("#signup_error").html("");
                    } else if (m_ID == "ResetPassword") {
                        const fpassError = document.querySelector("#fpass_error");
                        fpassError.style.display = "none";
                        fpassError.style.innerHTML = " ";
            ...
            if ((target.hasAttribute("data-dismiss") && target.getAttribute("data-dismiss") == "customPopup") || target.classList.contains("customPopup")) {
    ...
    closeIcon.addEventListener("click", function () {
        Document.querySelectorAll(".customPopup").classList.remove("active");
        Document.querySelectorAll(".commonPopup").classList.remove("active");
    ...
    mainTable.addEventListener("scroll", (e) => {
        ...
        let ListingTable = document.querySelectorAll(".ListingTable th:nth-child(3)");
        let ListingTable2 = document.querySelectorAll(".ListingTable td:nth-child(3)");
        if (isScroll) {
            ListingTable.classList.add("before");
            ListingTable2.classList.add("before");
        } else {
            ListingTable.classList.remove("before");
            ListingTable2.classList.remove("before");
        }
    });
    ...
    $(".currenciesTable, .exchangeTable").on("scroll", function (e) {
        let isScroll = e.currentTarget.scrollLeft;
        let ListingTable = document.querySelectorAll(".ListingTable th:nth-child(2)");
        let ListingTable2 = document.querySelectorAll(".ListingTable td:nth-child(2)");
        if (isScroll) {
            ListingTable.classList.add("before");
            ListingTable2.classList.add("before");
        } else {
    ...
    // .historical-data
    let historicalData = document.querySelectorAll(".historical-data");
    historicalData.addEventListener("scroll", (e) => {
        let isScroll = e.currentTarget.scrollLeft;
        let ListingTable = document.querySelectorAll(".ListingTable th:nth-child(1)");
        let ListingTable2 = document.querySelectorAll(".ListingTable td:nth-child(1)");
        if (isScroll) {
            ListingTable.classList.add("before");
            ListingTable2.classList.add("before");
        } else {
    ...
    // searchPopup
    let searchPopup = document.querySelectorAll(".header-for-mobile .searchPopup");
    ...
    searchPopupHider.addEventListener("click", () => {
    ...
    $(function () {
        ...
        let gototopIconDiv = document.querySelectorAll(".gototopIconDiv");
        gototopIconDiv.addEventListener("click", () => {
            document.querySelector("body").animate({ top: 0 }, {duration: 500});
        });
    ...
    popupRemover.addEventListener("click", () => {
        let LoginBox = document.querySelectorAll("#LoginBox");
    ...
    function moveThead() {
        ...
        window.setTimeout(function () {
            scrollElement.style.visibility = "visible";
            scrollElement.style.transition = "0.1s";
            scrollElement.style.boxShadow = "rgba(88,102,126,0.08) 0px 4px 24px, rgba(88,102,126,0.12) 0px 1px 2px";
        }, 500);
    }
...
function buildCurrencyList(required_char = "") {
    ...
    currencyJson.popular.forEach((data, index) => {
        if (required_char != "" && data.name.toLowerCase().indexOf(required_char) === -1) return;
        ...
        html +=
            ...
            `" onclick="top.location='${locurl}';">
					<img src="` +
            (data.crypto == 1 ? "/assets/currencies/32x32/" + data.id + ".png" : "/assets/fiats/" + data.symbol + ".svg") +
            `" alt="">
					<div class="rightText"><span>${data.name}</span><br> <small class="textUpperCase">${data.symbol} ` +
            (data.csymbol != null && data.csymbol != "" && data.symbol != data.csymbol ? " - " + data.csymbol : "") +
            `</small></div>
					</div>`;
    });
...
function buildLanguageList(required_char = "") {
    ...
    languageJson.forEach((data, index) => {
        if (required_char != "" && data.name_dest.toLowerCase().indexOf(required_char) === -1) return;
